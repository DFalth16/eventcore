...
@section('page_title', 'Editar Participante')
@section('content')
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px">
  <h2 style="font-weight:200">Editar <strong>Participante</strong></h2>
  <a href="/participantes" class="btn btn-g"><i class="bi bi-arrow-left"></i> Volver</a>
</div>
@if($errors->any())
  <div class="alert alert-error">@foreach($errors->all() as $e)<div>• {{ $e }}</div>@endforeach</div>
@endif
<div class="card" style="max-width:700px">
  <form method="POST" action="/participantes/{{ $participante->id_participante }}">
    @csrf @method('PUT')
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
      <div class="form-group">
        ...
        <input type="text" name="telefono" class="form-control" value="{{ old('telefono', $participante->telefono) }}">
      </div>
      <div class="form-group">
        <label>Documento ID (opcional)</label>
        <input type="text" name="documento_id" class="form-control" value="{{ old('documento_id', $participante->documento_id) }}">
      </div>
    </div>
