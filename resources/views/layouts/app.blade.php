{{-- Layout base con sidebar, topbar y CSS premium del sistema original --}}
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
.sbg.finalizado,.sbg.borrador{background:rgba(255,255,255,0.05);color:var(--t3)}
.sbg.cancelado{background:var(--rose-g);color:var(--rose)}
.sbg.agotado{background:var(--amber-g);color:var(--amber)}
.sbg.confirmada{background:var(--lime-g);color:var(--lime)}
.sbg.pendiente{background:var(--amber-g);color:var(--amber)}
.sbg.cancelada{background:var(--rose-g);color:var(--rose)}
...
.btn-r{background:var(--rose-g);color:var(--rose);border:1px solid rgba(255,77,109,.2)}
.btn-r:hover{background:rgba(255,77,109,.2);transform:translateY(-2px)}
...
.alert-success{background:var(--lime-g);color:var(--lime);border:1px solid rgba(163,230,53,.2)}
.alert-error{background:var(--rose-g);color:var(--rose);border:1px solid rgba(255,77,109,.2)}
.alert-info{background:var(--cyan-gs);color:var(--cyan);border:1px solid rgba(0,212,255,.2)}
</style>
@yield('extra_styles')
</head>
<body>
...
<div class="shell">
  ...
  <div class="main">
    <header class="tb">
      <div class="tb-title">@yield('page_title', 'Panel de Control')</div>
      <div style="font-size:13px;color:var(--t3)">
        {{ auth('admin')->user()->nombres ?? '' }} {{ auth('admin')->user()->apellidos ?? '' }}
      </div>
    </header>
    <div class="con">
      @if(session('success'))
